...
function xlsx(file, options) { 
	...
	var defaultFontName = options.defaultFontName || 'Cambria';
	var defaultFontSize = options.defaultFontSize || 9;
    ...
    fonts = new Array(1);
    	...
    	merges = [];
            ...
            val = escapeXML(val);
					...
					if (columns[j] == null) {
						columns[j] = {autoWidth: true, max:0};
					}
					if (cell.autoWidth) {
			...
			s = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac">'
				...
				+ '</sheetData>';
			debugger;
			if (worksheet.cfs !== undefined) {
				
				var dxfIds = {green : 4, red : 2};
				var operators = {gt: 'greaterThan', lt : 'lessThan'};
				
				s +=	Object.keys(worksheet.cfs).map(function(field){
						return Object.keys(worksheet.cfs[field]).map(function(op, i){
							var o = worksheet.cfs[field][op];
							return '<conditionalFormatting sqref="'+o.range+'">'+
								'<cfRule type="cellIs" dxfId="'+dxfIds[o.frmt]+'" priority="'+(i+1)+'" operator="'+operators[op]+'">'+
								'<formula>'+(o.th || 0)+'</formula>'+
								'</cfRule></conditionalFormatting>';
							}).join('');
						}).join('');
				
				/* s +=	worksheet.cfs.map(function(cf, i){
							return '<conditionalFormatting sqref="+cf.range+">'+
								(
									(cf.gt == 1) ? '<cfRule type="cellIs" dxfId="4" priority="2" operator="greaterThan">' :
									'<cfRule type="cellIs" dxfId="2" priority="1" operator="lessThan">'
								) +
								'<formula>'+cf.th || 0+'</formula>'+
								'</cfRule></conditionalFormatting>';
						}).join('');
				*/
			}
		...
		xl.file('styles.xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac">'
			...
			+ styles.join('') + '</cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles><dxfs count="6"><dxf><font><condense val="0"/><extend val="0"/><color rgb="FF9C0006"/></font><fill><patternFill><bgColor rgb="FFFFC7CE"/></patternFill></fill></dxf><dxf><font><condense val="0"/><extend val="0"/><color rgb="FF006100"/></font><fill><patternFill><bgColor rgb="FFC6EFCE"/></patternFill></fill></dxf><dxf><font><condense val="0"/><extend val="0"/><color rgb="FF9C0006"/></font><fill><patternFill><bgColor rgb="FFFFC7CE"/></patternFill></fill></dxf><dxf><font><condense val="0"/><extend val="0"/><color rgb="FF006100"/></font><fill><patternFill><bgColor rgb="FFC6EFCE"/></patternFill></fill></dxf><dxf><font><condense val="0"/><extend val="0"/><color rgb="FF006100"/></font><fill><patternFill><bgColor rgb="FFC6EFCE"/></patternFill></fill></dxf><dxf><font><condense val="0"/><extend val="0"/><color rgb="FF006100"/></font><fill><patternFill><bgColor rgb="FFC6EFCE"/></patternFill></fill></dxf></dxfs>'
